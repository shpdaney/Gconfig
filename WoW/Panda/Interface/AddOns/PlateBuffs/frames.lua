...
--		self.skin:SetBlendMode(blendmode) 
	...
	--	self.border:SetTexture("Interface\\Addons\\PlateBuffs\\texture\\border.tga")
	
	--	ntexture = P.borderTexture --"Interface\\Addons\\PlateBuffs\\texture\\border.tga"
	...
	if self.isDebuff then
		...
		if colour then
			if P.colorByType then
				...
				if colour == "Magic" then color = P.color2 end
				if colour == "Curse" then color = P.color3 end
				if colour == "Disease" then color = P.color4 end
				if colour == "Poison" then color = P.color5 end
				if colour == "none" or colour == "" then color = P.color1 end
...
--~ 	f.cdtextureP:SetPoint("TOP", f)
	...
	f.cdtexture = CreateFrame("Cooldown", "MainFrameTexture" , f.icon)
...
--	if LBF then
	...
	--	f.skin:SetBlendMode("ADD") 
		...
		f.msqborder.bordersize = 0
		f.msqborder.normalsize = 0
...
--~ 			buffBars[plate][r]:Show() --***
	...
	if not plate.PlateBuffsIsHooked then
		...
		plate:HookScript("OnSizeChanged",
			function(self, w, h)
